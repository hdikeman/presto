        ...
        implements Rule<ProjectNode>
{
    ...
    public Result apply(ProjectNode node, Captures captures, Context context)
    {
        ...
        for (Entry<VariableReferenceExpression, AggregationNode.Aggregation> entry : child.getAggregations().entrySet()) {
            ...
            // Get the output type of the aggregation
            Type outputType = aggregation.getCall().getType();

            // Replace all references to this aggregation output with NULL constants
            for (Entry<VariableReferenceExpression, RowExpression> outputEntry : outputs.entrySet()) {
                RowExpression outputExpression = outputEntry.getValue();
                if (outputExpression.equals(variable)) {
                    // This output directly references the aggregation result, replace with NULL
                    outputsV2.put(outputEntry.getKey(), constantNull(outputType));
                }
            }

            // Remove the aggregation since it's been replaced with NULL
    ...
    private boolean replaceable(AggregationNode.Aggregation aggregation)
    {
        ...
        return functionResolution.isMaxFunction(handle) ||
                ...
                functionResolution.isAverageFunction(handle) ||
                functionResolution.isSumFunction(handle) ||
